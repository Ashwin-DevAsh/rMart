...
class ProductsApi {
  ...
  static Future<List> getProducts() async {
    ...
    var uriResponse = await client.get(
      ApiContext.martURL + "/getAllProducts",
      headers: {
  ...
  static Future<List> getMerchants() async {
    var client = http.Client();
    var uriResponse = await client.get(
      ApiContext.profileURL + "/getMerchants",
      headers: {
        "key": await FirebaseRempteConfig.getServerKey(),
        "Content-Type": "application/json",
      },
  ...
  static getMyOrders({status}) async {
    var client = http.Client();
    var uriResponse = await client.get(
      ApiContext.martURL +
  ...
  static loadDatas() async {
    ...
    products.forEach((element) {
      if (categories.contains(element["category"])) {
        data[element["ownerid"]][element["category"]].insert(0, element);
      } else {
        categories.forEach((category) {
