...
class _DisclimerState extends State<Disclimer> {
        ...
        child: Row(children: [
          ...
          GestureDetector(
            onTap: () {
              try {
                launch(
                    "mailto:rMart.support@rajalakshmi.edu.in?subject=rMart Help");
              } catch (e) {}
            },
            child: Text(
              "Need help?",
  ...
  bool isValidInput(context) {
    ...
    if (collegeID.text.length < 2) {
      Scaffold.of(context).showSnackBar(SnackBar(
        content: Text("Invalid College ID"),
      ));
      return false;
    }
    ...
    if (password.text.length < 8) {
      Scaffold.of(context).showSnackBar(SnackBar(
        content: Text("Password must be atleast 8 character"),
      ));
      return false;
    }
    return true;
