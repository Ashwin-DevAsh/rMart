    ...
    return Scaffold(
      ...
      body: Consumer<OrderListModel>(
        builder: (context, orderNotifier, _) => FutureBuilder(
            future:
                ProductsApi.getMyOrders(status: categories[selectedCategory]),
                ...
                child: SingleChildScrollView(
                  ...
                  child: Column(
                    ...
                    children: [
                          ...
                          SizedBox(height: 25),
                          HelperWidgets.getCategory(
                              context, categories, selectedCategory, (index) {
                            setState(() {
  ...
  List<Widget> getOrderList(orders, orderNotifier) {
    if (isLoaded) {
      if (categories[selectedCategory] == "All") {
        return List.generate(orders.length,
            (index) => orderTile(context, orders[index], orderNotifier));
      } else if (categories[selectedCategory] == "Pending") {
        var pendingOrders =
            orders.where((o) => o["status"] == "pending").toList();
        return List.generate(pendingOrders.length,
            (index) => orderTile(context, pendingOrders[index], orderNotifier));
      } else if (categories[selectedCategory] == "Delivered") {
        var expOrders =
            orders.where((o) => o["status"] == "delivered").toList();
        return List.generate(expOrders.length,
            (index) => orderTile(context, expOrders[index], orderNotifier));
      } else {
      ...
      child: Center(
        ...
        children: [
          Padding(
            ...
            child: Image(
              image: Image.asset("lib/assets/Images/empty_orders.png").image,
          ...
          Navigator.push(
              ...
              CupertinoPageRoute(
                  builder: (ctx) => OrderDetails(
                      order: order,
                      orderNotifier: orderNotifier))).then((value) {
                        ...
                        Container(
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              Text(
                                OrderApi.getDate(order["timestamp"]),
                                style: TextStyle(
